...
int main()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(third_cube_vertices), third_cube_vertices, GL_STATIC_DRAW);

	glBindVertexArray(cubeVAOs[2]);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	...
	// Загрузка текстур
	unsigned int diffuseMap = loadTexture("textures/latune.jpg");
	unsigned int specularMap = loadTexture("textures/latune.jpg");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::mat4(1.0f);
		model = glm::mat4(1.0f);
		model = glm::translate(model, second_cube_positions[0]);
		float angle = 20.0f * 0;
		model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
		...
		model = glm::mat4(1.0f);
		model = glm::mat4(1.0f);
		model = glm::translate(model, third_cube_positions[0]);
		model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
